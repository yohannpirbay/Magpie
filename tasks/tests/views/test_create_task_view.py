...
class CreateTasksTestCase(TestCase):
    ...
    def test_create_task_with_blank_user(self):
        ...
        response = self.client.post(self.url, data)
        self.assertFormError(response, 'form', 'assigned_users', 'This field is required.')

    def test_create_task_with_blank_due_date(self):
        self.client.force_login(self.user)
        data = {'title': 'New Task', 'description': 'New Description', 'assigned_users': self.user.id, 'due_date': ''}
        response = self.client.post(self.url, data)
        self.assertFormError(response, 'form', 'due_date', 'This field is required.')

    def test_create_task_with_blank_team(self):
        self.client.force_login(self.user)
        data = {'title': 'New Task', 'description': 'New Description', 'assigned_users': self.user.id, 'due_date': '2024-02-29', 'team': ''}
        response = self.client.post(self.url, data)
